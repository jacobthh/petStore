...
import './Verify.css'; // Optional: Include custom CSS for styling
...
const Verify = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://localhost:3001/api/adm');
                if (!response.ok) {
    ...
    const handleVerify = (user) => {
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error('Failed to verify entry');
            }
        ...
        .then(data => {
            ...
            // Here you can update petData state if needed to reflect the change
            // e.g., setPetData(petData.map(p => (p.email_id === user.email_id ? { ...p, status: 1 } : p)));
        })
        .catch(error => {
            ...
            <div className="carousel-item">
                ...
                <div>
                    ...
                    <div className="button-group">
                        <Button onPress={() => handleVerify(user)} color="primary" auto>
                            Verify
                        </Button>
